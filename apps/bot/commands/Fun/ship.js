import { percentageBar } from "@yurna/util/functions/util";
...
export default {
 ...
 run: async (client, interaction, guildSettings) => {
  try {
   ...
   if (!first || !second) {
    return await client.errorMessages.createSlashError(interaction, "❌ You need to specify two users to ship");
   }

   if (first.id === second.id) {
    return await client.errorMessages.createSlashError(interaction, "❌ You can't ship the same user with themselves");
   }
