...
import { cacheGet, cacheSet, cacheTTL } from "@yurna/database/redis";
import { createUser } from "@yurna/util/database";
...
export async function interactionCreate(client: Yurnabot, interaction: Interaction): Promise<Message | void> {
 try {
  client.commandsRan++;
