  ...
  if (roleConfig) {
   ...
   if (roleConfig.exclusive) {
    for (const otherRoleConfig of channelConfig[reaction.message.id]) {
     if (otherRoleConfig.emoji !== reaction.emoji.toString()) {
      ...
      const oldReaction = reaction.message.reactions.cache.find((r) => r.emoji.toString() === otherRoleConfig.emoji);
   ...
   if (logChannel) {
    const logEmbed = new EmbedBuilder()
     ...
     .setColor("Green")
     .addFields({ name: "Server", value: reaction.message.guild.name, inline: true }, { name: "Benutzer", value: `${user.tag}`, inline: true }, { name: "Rollen", value: addedRoles.join(", ") || "Keine", inline: false }, { name: "Zeitpunkt", value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true })
