...
export async function getUserAvatar(client: Yurnabot, interaction: ChatInputCommandInteraction, color: ColorResolvable) {
 try {
...
export async function getUserBanner(client: Yurnabot, interaction: ChatInputCommandInteraction, color: ColorResolvable) {
 try {
  if (!interaction.guild) return client.errorMessages.createSlashError(interaction, "❌ This command can only be used in a server.");
  const user = interaction.options.getMember("user") as GuildMember;
