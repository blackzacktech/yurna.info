 ...
 * @param {string} type - Type of giveaways to find (ended or running)
   ...
   .setDescription(
    `>>> ${
     list.length > 0 // prettier
      ? list
         .map((g) => {
          const prize = g.data.prize ? g.data.prize.replace(`${client.config.emojis.giveaway} Giveaway: `, "").replace(`${client.config.emojis.giveaway} Drop: `, "") : "No prize";
          const endsAt = g.data.endAt ? `, Ends <t:${Math.floor(g.data.endAt / 1000)}:R>` : "No end date";
          return `**${prize}**: Winners: \`${g.data.winnerCount || 1}\`${g.data.ended ? "" : endsAt}, https://discord.com/channels/${g.guildId}/${g.data.channelId}/${g.messageId}`;
         })
         .join("\n")
