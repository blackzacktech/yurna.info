...
const DeleteMessage = ({ actions, setActions }: DeleteMessageProps) => {
 const setDelete = () => {
  ...
  const updatedActions = actions.some((action) => action.type === deleteActionType) ? actions.filter((action) => action.type !== deleteActionType) : [...actions, { type: deleteActionType, metadata: { custom_message: "Message blocked due to containing an invite link. Rule added by Yurna.info" } }];
