...
export async function POST(request: NextRequest) {
 try {
   ...
   data.actions = [
    {
     ...
     metadata: {
      custom_message: "Message blocked due to containing an link. Rule added by Yurna.info",
     },
    },
   ];
  }
  ...
  const validatedActions = await validateAutoModRuleActions(data.actions, guildChannels, "Message blocked due to containing an link. Rule added by Yurna.info");
  ...
  const createdRule = await createDiscordAutoModRule(server.id, "anti-link", {
   ...
   name: "Disallow links [Yurna.info]",
