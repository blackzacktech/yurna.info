...
import { createLog } from "@yurna/util/database";
import { AutoModerationRuleCreationData, createDiscordAutoModRule, validateAutoModIgnores, validateAutoModRuleActions } from "@yurna/util/functions/automod";
import { getGuild, getGuildChannels, getGuildFromMemberGuilds, getGuildRoles } from "@yurna/util/functions/guild";
...
export async function POST(request: NextRequest) {
 try {
   ...
   data.actions = [
    {
     ...
     metadata: {
      custom_message: "Message blocked due to containing an invite link. Rule added by Yurna.info",
     },
    },
   ];
  }
  ...
  const validatedActions = await validateAutoModRuleActions(data.actions, guildChannels, "Message blocked due to containing an invite link. Rule added by Yurna.info");
  ...
  const createdRule = await createDiscordAutoModRule(server.id, "anti-invite", {
   ...
   name: "Disallow invites [Yurna.info]",
